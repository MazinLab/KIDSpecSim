...
class Engine:
    def __init__(self, spectrograph: SpectrographSetup):
        ...
        print("\nConfigured engine for convolving, drawing photons, and fitting.")
    ...
    def gauss_summed(self, x, *args):
        ...
        return np.sum(self.gauss(x[:, None], mu, sig, A), axis=0)

    def quad_formula(self, a, b, c):
        """
        :return: positive quadratic formula result for ax^2 + bx + c = 0
        """
        return (-b + np.sqrt(b ** 2 - 4 * a * c)) / (2 * a)

    def gauss_intersect(self, mu, sig, A):
        """
        ...
        return self.quad_formula(np.array(a), np.array(b), np.array(c))
