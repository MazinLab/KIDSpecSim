...
class MKIDSpreadFunction:
    def __init__(self, bin_edges=None, cov_matrix=None, orders=None, filename: str = '', metadata=None):
        """
        ...
        :param metadata: additional identifying data for the spectrograph and conditions for calculation
        """
    ...
    def save(self, filename=''):
        ...
        np.savez(fn, bin_edges=self.bin_edges, cov_matrix=self.cov_matrix, metadata=self.metadata)

    def _load(self):
        msf = np.load(self.filename)
        ...
        assert msf['metadata'], 'File is missing meta data or syntax is incorrect.'
        self.metadata = msf['metadata']
...
if __name__ == '__main__':
    msf = MKIDSpreadFunction(bin_edges=np.ones([6,2048]), cov_matrix=np.ones([5,5,2048]), orders=[5,6,7,8,9], metadata=['blah'])
