...
*SET new_spetrograting=True IF CHANGING ANY FUNDAMENTAL PROPERTIES OF SPECTROGRAPH OR GRATING.
*CHOOSE WHETHER TO GENERATE CALIBRATION DATA OR NOT (DEFAULT).
...
if __name__ == '__main__':
    ...
    if new_spectrograting:
        ...
        print(f"\nConfigured new, non-default spectrometer and observation specifications."
              f"\n\tExposure time: {exptime}"
              f"\n\tPhotons per pixel limit: {pixel_lim}"
        ...
        grating = GratingSetup(l0, m0, pixel_size, npix, focal_length, littrow=littrow)
        spectrograph = SpectrographSetup(grating, detector, m0, m_max, l0, pixels_per_res_elem,
                                         focal_length, littrow=littrow)
    else:
        print(f"\nConfigured observation specifications."
              f"\n\tExposure time: {exptime}"
              f"\n\tPhotons per pixel limit: {pixel_lim}")
        grating = GratingSetup()
        detector = MKIDDetector(generate_R0=gen_cal)
    ...
    if full_convolution:
        ...
        result_wave, result, mkid_kernel = \
            engine.convolve_mkid_response(inbound.waveset, broadened_spectrum, *sampling_data,
    ...
    if plot_int:
        ...
        fluxden = np.empty(
            [5, 2048])  # regaining flux density by summing flux by previous indices and multiplying by dx
        for j in range(5):
            for i in range(2047):
    ...
    t_photons, l_photons = engine.draw_photons(result_wave, result, exptime=exptime, limit_to=pixel_lim,
                                               plot_int=plot_int)
