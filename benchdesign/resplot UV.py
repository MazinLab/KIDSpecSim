...
def mkidr(l, Rlam=100, lam=100):
    return Rlam*lam/l


def osep(l, Rlam=25, c=1.33, lam=100):
    return c*l**2/Rlam/lam

fsr = lambda l, m: l/m


def res(l, owid, npix=2048, nspot=2.15):
...
ref_lam=ref_lam2-osep(ref_lam2,Rlam=Rlam,c=2.1)
ref_lam3=ref_lam2-osep(ref_lam2,Rlam=Rlam,c=1.33)
fwhmspace=2.1
fwhmspace2=1.33
Rlam=100
lam=np.linspace(85,126,1000)
plt.figure(figsize=(3.5,6))
g800 = models.Gaussian1D(amplitude=1, mean=ref_lam, stddev=osep(ref_lam,Rlam=Rlam,c=1)/2.355)
g800n = models.Gaussian1D(amplitude=1, mean=ref_lam2, stddev=osep(ref_lam2,Rlam=Rlam,c=1)/2.355)
...
plt.ylabel('Counts')
plt.gca().set_yticklabels([])
plt.text(130,.95,'$N_{FWHM}=2.1$')
plt.subplot(2,1,2)

g800 = models.Gaussian1D(amplitude=1, mean=ref_lam2, stddev=osep(ref_lam2,Rlam=Rlam,c=1)/2.355)
g800n = models.Gaussian1D(amplitude=1, mean=ref_lam3, stddev=osep(ref_lam3,Rlam=Rlam,c=1)/2.355)
plt.plot(lam,g800(lam),'-.k')
plt.plot(lam,g800n(lam),'--k')
plt.plot(lam,(g800+g800n)(lam))
plt.xlabel('Wavelength (nm)')
...
ord=s.split('\n')
dat=np.array([list(map(float,o.split())) for o in ord])
